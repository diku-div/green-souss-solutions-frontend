...
const Button: FC<ButtonProps> = ({variante='primary',isfullwidth,children}) => {
    ...
    <div className="relative group w-full  flex items-center justify-center ">
      <div className="relative w-64 h-14 opacity-90 overflow-hidden rounded-4xl bg-black/50 z-10">
        ...
        <div className="absolute flex items-center justify-center text-white z-[1] opacity-90 rounded-4xl inset-0.5 bg-black/50">
          <p  className="input font-semibold text-lg h-full opacity-90 w-full px-16 py-3 rounded-4xl bg-black/50">
            {children}
          </p>
        </div>
  ...
  <div>
    <button type='submit' className={`px-8 py-2 text-white bg-green-600 rounded-full shadow-lg transition-transform transform hover:scale-105 focus:outline-none ${isfullwidth ? 'w-full' : ''}`}>
      {children}
      </button>
  </div>
